        ...
        onPlayerInteract: (e) => {
            ...
            if (block?.typeId === `terra:deepslate_crate_locked` && lockedState === 0) {
                if (selectedItem?.typeId === `terra:deepslate_key`) {
                    ...
                    dimension.runCommand(`particle terra:chestlock_open ${location.x} ${location.y} ${location.z}`);

                    if (chance <= 2 / 3) { // spawn loot and drop an empty crate variant
                        dimension.runCommand(`setblock ${location.x} ${location.y} ${location.z} air`);
                        dimension.runCommand(`loot spawn ${location.x} ${location.y} ${location.z} loot "chestlock/deepslate_locked"`);
                        dimension.runCommand(`loot spawn ${location.x} ${location.y} ${location.z} loot "chestlock/deepslate_empty"`);
                    } else if (chance <= 1 / 3) {  // a third chance of turning into a chesthound
                        dimension.runCommand(`setblock ${location.x} ${location.y} ${location.z} air`);
                        dimension.runCommand(`summon terra:chesthound_deepslate ${location.x} ${location.y} ${location.z}`);
                    }
                } else if (selectedItem.typeId !== `terra:deepslate_key`) {
                    // remind the player that the crate is locked
                    dimension.runCommand(`function notif/crate_is_locked`);
                }
            ...
            if (block?.typeId === `terra:outsider_crate_locked` && lockedState === 0) {
                if (selectedItem?.typeId === `terra:outsider_key`) {
                    ...
                    if (chance <= 2 / 3) { // spawn loot and drop an empty crate variant
                        dimension.runCommand(`setblock ${location.x} ${location.y} ${location.z} air`);
                        dimension.runCommand(`loot spawn ${location.x} ${location.y} ${location.z} loot "chestlock/outsider_locked"`);
                        dimension.runCommand(`loot spawn ${location.x} ${location.y} ${location.z} loot "chestlock/outsider_empty"`);
                    } else if (chance <= 1 / 3) {  // a third chance of turning into a chesthound
                        dimension.runCommand(`setblock ${location.x} ${location.y} ${location.z} air`);
                        dimension.runCommand(`summon terra:chesthound_outsider ${location.x} ${location.y} ${location.z}`);
                    }
            ...
            if (block?.typeId === `terra:chesthound_block`) {
                dimension.runCommand(`particle terra:chestlock_open ${location.x} ${location.y} ${location.z}`);
                dimension.runCommand(`setblock ${location.x} ${location.y} ${location.z} air`);
                dimension.runCommand(`summon terra:chesthound ${location.x} ${location.y} ${location.z}`);
